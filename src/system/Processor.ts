...
type Keyboard = import("./Keyboard").Keyboard;
...
export class Processor {
  ...
  private formatInstruction(instruction: Instruction | null) {
    ...
    return Object.entries(instruction).reduce((accumulator, [key, value]) => {
  ...
  performCycle() {
    ...
    switch (nibble0) {
      case 0x0: {
        switch (byte1) {
          case 0xe0: {
            for (let i = 0; i < display.width * display.height; i += 1) {
              display.set(i, 0);
            }
            break;
          }
      ...
      case 0x5: {
        if (memory.getRegisterV(nibble1) === memory.getRegisterV(nibble2)) {
          memory.incrementRegisterProgramCounter(2);
        }
        break;
      }
      ...
      case 0x8: {
        switch (nibble3) {
          case 0x0: {
            memory.setRegisterV(nibble1, memory.getRegisterV(nibble2));
            break;
          }
          ...
          case 0x2: {
            memory.setRegisterV(
              nibble1,
              memory.getRegisterV(nibble1) & memory.getRegisterV(nibble2),
            );
            break;
          }
      ...
      case 0xc: {
        // 0 - 255
        const randomNumber = Math.floor(Math.random() * Math.floor(256));
      ...
      case 0xd: {
        ...
        for (let yOffset = 0; yOffset < spriteByteCount; yOffset += 1) {
          ...
          for (let xOffset = 0; xOffset < 8; xOffset += 1) {
            ...
            const pixel = (spriteByte & (0x80 >>> xOffset)) >>> (7 - xOffset);
      ...
      case 0xe: {
        switch (byte1) {
          case 0x9e: {
            if (this.keyboard.isKeyPressed(this.memory.getRegisterV(nibble1))) {
              memory.incrementRegisterProgramCounter(2);
            }
            break;
          }
          ...
          case 0xa1: {
            if (
              !this.keyboard.isKeyPressed(this.memory.getRegisterV(nibble1))
            ) {
              memory.incrementRegisterProgramCounter(2);
            }
            break;
      ...
      case 0xf: {
        switch (byte1) {
          ...
          case 0x1e: {
            memory.setRegisterI(
              memory.getRegisterI() + memory.getRegisterV(nibble1),
            );
            break;
          }

          case 0x29: {
            memory.setRegisterI(nibble1 * 5 + memory.getHexSpritesOffset());
            break;
          }
          ...
          case 0x33: {
            ...
            for (let i = 2; i >= 0; i -= 1) {
              ...
              memory.setMemoryByte(
                memoryOffset,
                (memory.getMemoryByte(memoryOffset) + (value % 10)) << 4,
              );
              value = (value / 10) | 0;
            }
            break;
          ...
          case 0x55: {
            ...
            for (let i = 0; i <= nibble1; i += 1) {
              memory.setMemoryByte(ri + i, memory.getRegisterV(i));
            }
            break;
          }
          ...
          case 0x65: {
            ...
            for (let i = 0; i <= nibble1; i += 1) {
              ...
              memory.setRegisterV(i, memory.getMemoryByte(memoryOffset));
            }
            break;
          }
